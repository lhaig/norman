...
7. **Handle gaps** — Offer via AskUserQuestion:
...
**The orchestrator, via the `Skill` tool. Never a worker.** Two independent reasons:
...
| `offer` | Ask once via AskUserQuestion: run it now / skip / turn off for this project. On "turn off", write `sweep_mode: off` to config.md and commit with the sweep. |
...
2. Offer via AskUserQuestion to turn accepted gaps into tasks — a new phase in `prds/TASKS.md` with PRDs in `prds/backlog/`, same shape as Mode 5 step 7. Which gaps are worth a test is a priority call and belongs to the user.
...
| `norman sweep` | Run the sweep skill against the last collapsed phase |
